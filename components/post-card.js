...
function PostCard({ title, category: source, excerpt, slug, author, date }) {
  ...
  return (
    <div className="w-60 relative bg-light dark:bg-dark group m-auto transition-all h-80 border-4 rounded-3xl overflow-hidden border-dark dark:border-dark">
      ...
      <div
        style={{
          clipPath: `circle(${state === true ? '150%' : '35%'} at -4% -4%)`,
        }}
        className="bg-dark dark:bg-darker text-light absolute transition-all top-0 left-0 w-full h-full"
      >
        <span
          onClick={() => handleClick()}
          className="font-bold cursor-pointer  text-2xl absolute px-6 py-4"
        >
          i
        </span>
        ...
        <div className="flex justify-between items-center flex-col h-60">
          <Tag tagName={source?.title} author={author} />
          <div className="flex justify-center items-center flex-col scale-90">
            <Date dateString={date} className="mb-4" />
